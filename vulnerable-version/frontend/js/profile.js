...
async function loadProfile() {
    try {
        ...
        if (accountCurrentEmail) accountCurrentEmail.textContent = user.email || "—";
...
function targetsInternalAdminRoute(rawUrl) {
    try {
        const parsed = new URL(rawUrl, window.location.origin);
        const loopbackHosts = ["127.0.0.1", "localhost", "::1", "[::1]"];
        return loopbackHosts.includes(parsed.hostname) && parsed.pathname.startsWith("/api/admin");
    } catch {
        return false;
    }
}
...
avatarImportForm?.addEventListener("submit", async (event) => {
    ...
    try {
        ...
        if (response.ok && targetsInternalAdminRoute(url)) {
            showToast("Internal admin session obtained - opening Admin Panel", "success");
            setTimeout(() => {
        ...
        showToast("Failed to import image from that URL", "error");
    } finally {
        if (importBtn) {
